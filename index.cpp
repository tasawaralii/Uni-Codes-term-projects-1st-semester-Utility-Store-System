                ...
                case 3: // Admin Moderators Menu
                    ...
                    while (true)
                    {
                        ...
                        case 0:
                            ...
                            while (true)
                            {
                                ...
                                if (flag)
                                {
                                    ...
                                    {
                                        cout << "\n\n\t\t";
                                        nes1 = intValidation("Moderator ID", "", 1, moderIDcounter);
                        ...
                        case 1:
                            ...
                            addmoderator(moderators, moderIDcounter, MAX_MODERATORS);
                        ...
                        case 3:
                            ...
                            if (flag)
                            {
                                ...
                                if (yesnt())
                                {
                                    ...
                                    cout << "\n\n\t\t";
                                    tem = intValidation("Moderator ID", "", 1, moderIDcounter);
...
void viewRequests(moderator moderators[], report reports[], int MAX_MODERATORS, int MAX_REPORTS, int status)
{
    ...
    if (counter != 0)
    { // if request available then check it
...
void showHistory(purchase purchases[], item_purchase item_purchases[], int custID, product products[], int MAX_PRODUCTS, int MAX_PURCHASES, int &item_purCount)
{
    ...
    for (int i = 0; i < MAX_PURCHASES; i++)
    {
        ...
        if (purchases[i].customerID == custID)
        { // Check CustomerID in Purchases
            if (noPurchase)
                cout << "\t\t\t┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓" << endl; // 66
            else
                cout << "\t\t\t┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫" << endl; // 66
            ...
            for (int j = 0; j < item_purCount; j++)
            {
                ...
                if (item_purchases[j].purchaseID == i)
                { // check purchase ID
...
void showmoderatorProfile(moderator moderators[], int moderID, int MAX_MODERATORS)
{
    ...
    for (int i = 0; i < MAX_MODERATORS; i++)
    {
        ...
        { // Check if the moderator record is valid and matches the given ID
...
void buyItems(product products[], purchase purchases[], item_purchase item_purchases[], int custID, int MAX_PRODUCTS, int MAX_PURCHASES, int &purchaseCount, int &item_purCount, int productCount)
{
    ...
    while (true)
    {
        ...
        if (index == -1 || products[index].stock == 0)
        {
            ...
            setcolor(7);
            continue;
        } else if(tem_stock[index] == 0) {
            setcolor(4);
            cout << "\n\n\n\t\tWe are out of stock" << endl;
            setcolor(7);
            continue;
        }
...
void updatemoderator(moderator moderators[], int moderID, int MAX_MODERATORS)
{
    ...
    if (index != -1)
    {
        ...
        while (true)
        { // show updated details each time it changes
    ...
    while (true)
    {
        ...
        if (ch == 8)
        {
            if (tem.length() == 0) // if user press backsapce and nothing typed
                continue;
            if (lastChar(tem) == '@')
            { // if erasing character is @ then mark at flag as false
            ...
            {
                if (lastChar(tem) == '.')
                { // if @ is encountered and erasing character is '.' then mark dotAfter@ to false
                    dotAat = false;
                    tem = popOut(tem);
                    cout << "\b \b";
                    continue;
                ...
                { // if there is no '.' after @ just remove character
                    tem = popOut(tem);
                    cout << "\b \b";
                    continue;
                }
                else if (dotAat)
                { // if there is '.' after @ then decrease after dot count
                    adot--;
                    tem = popOut(tem);
                    cout << "\b \b";
                    continue;
        ...
        if (ch == '.')
        {
            ...
            if (lastChar(tem) == '.' || lastChar(tem) == '@') // two consective dots and dot next to @
                continue;
            if (at)
            { // if @ is present and '.' occurs mark dotAfter@ as true
                cout << ch;
                tem += ch;
                dotAat = true;
                continue;
...
string getString(int limit, bool hide, bool alphaOnly)
{
    ...
    while (true)
    {
        ...
        if (str.length() == limit)
        { // if max limit is reached only can submit
            if (c == 13)
                break;
            else if (c != 8)
                continue;
...
void cursorPosition(int x, int y)
{ // manipulate the position of cursor
    COORD c;
...
string postfixInt(int n, string str)
{ // return string by adding prefix
    return intToString(n) + ' ' + str;
}

string giveReviewStatus(int status)
{ // return review status string
    if (status == 1)
        return "Approved";
    if (status == 2)
        return "Disapproved";
...
void initializeByIndex(int arr[], int size)
{ // initialixe array with index number
    for (int i = 0; i < size; i++)
        arr[i] = i;
}
...
string centerString(string str, int width)
{ // add leading and trailing spaces to center string in given space
...
string makeIDString(string string, int ID)
{ // add prefix and pad by adding '0's
...
{ // add '0's in start to fill width
...
int loginCustomer(customer customers[], int MAX_CUSTOMERS)
{
    ...
    for (int i = 0; i < MAX_CUSTOMERS; i++)
        if (customers[i].flag && isSame(CNIC, customers[i].CNIC))
        { // match CNIC
...
int *returnBuyedProducts(int custID, purchase purchases[], item_purchase item_purchases[], int purchaseCount, int item_purCount, int MAX_PRODUCTS, int &distinctCounter)
{
    ...
    { // loop over purchases
        ...
        if (purchases[i].customerID == custID)
        { // if purchase is of requested customer

            for (int j = 0; j < item_purCount; j++)
            { // loop over items buyed in that purchase

                if (item_purchases[j].purchaseID == i)
                { // check if this item is of that purchase

                    bool duplicate = false; // consider it as unique

                    for (int k = 0; k < distinctCounter; k++)
                    { // loop over unique IDs array

                        if (distinctIDs[k] == item_purchases[j].productID)
                        { // if ID is already present then mark as duplicate
...
int *returnReviewedProducts(int custID, review reviews[], int reviewCount, int productCount, int &distinctCounter)
{
    ...
    { // loop over reviews
        ...
        if (reviews[i].type == 2 && reviews[i].customerID == custID)
        { // if review is of requested user

            bool duplicate = false;

            for (int k = 0; k < distinctCounter; k++)
            { // check productID of the review with unique IDs array
              ...
              report reports[], int MAX_REPORTS)
{
    ...
    ifstream repin("data/requests.csv");
    IDcounter = 0;
    ...
    while (repin)
    {
        ...
        repin >> reports[IDcounter].status;
        repin.ignore();
    ...
    ifstream revin("data/reviews.csv");
    IDcounter = 0;
    ...
    while (revin)
    {
        ...
        revin.ignore();
        revin >> reviews[IDcounter].type;
        revin.ignore();
        revin >> reviews[IDcounter].moderatorID;
        revin.ignore();
        revin >> reviews[IDcounter].productID;
        revin.ignore();
        revin >> reviews[IDcounter].customerID;
        revin.ignore();
        revin >> reviews[IDcounter].rating;
        revin.ignore();
    ...
    ifstream itemin("data/item-purchase.csv");
    ...
    ifstream purin("data/purchases.csv");
    ...
    ifstream custin("data/customers.csv");
    IDcounter = 0;
    ...
    while (custin)
    {
        ...
        custin >> customers[custIDcounter].ID;
        custin.ignore();
    ...
    ifstream moderin("data/moderators.csv");
    IDcounter = 0;
    ...
    while (moderin)
    {
        ...
        if (moderators[moderIDcounter].first_name == "")
            break;
        ...
        moderin >> moderators[moderIDcounter].salary;
        moderin.ignore();
        moderin >> moderators[moderIDcounter].ID;
        moderin.ignore();
    ...
    ifstream proin("data/products.csv");
    IDcounter = 0;
               ...
               report reports[], int MAX_REPORTS)
{
    ...
    ofstream repout("data/requests.csv");
    ...
    ofstream revout("data/reviews.csv");
    ...
    ofstream itemout("data/item-purchase.csv");
    ...
    ofstream purout("data/purchases.csv");
    ...
    ofstream custout("data/customers.csv");
    ...
    ofstream moderout("data/moderators.csv");
    ...
    ofstream prout("data/products.csv");
